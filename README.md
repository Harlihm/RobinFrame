...
The app is a single Streamlit file (`robinframe.py`) driving an AG2 agent swarm. Your form inputs are packed into one task prompt, then four agents pass the work around a ring, each building on the previous agent's output:
...
5. **Output** — The four sections appear in expanders, and the final script is shown in a copyable code block with a `.txt` download button, ready to paste into Seedance, Higgsfield, Kling, Runway, or Pika. When Seedance is selected, each shot is written as a self-contained natural-language prompt paragraph with explicit camera moves, matching Seedance's prompt conventions.
...
1. **Clone the Repository**:
   ...
   ```bash
   git clone https://github.com/Harlihm/RobinFrame.git
   cd RobinFrame
   ```
...
3. **Set Up OpenAI API Key**:
   ...
   - Copy `.env.example` to `.env` and add your key (`.env` is gitignored). The app loads it at startup — there is no key input in the UI.

4. **Run the Streamlit App**:

   ```bash
   streamlit run robinframe.py
   ```

## Usage

1. Describe your video in the concept brief (or leave it empty for the default scenario)
2. Pick a genre skin, video length, art style, target tool, and mood notes
3. Click "Generate Script" and watch the crew's summaries land in the sidebar
4. Review the beat sheet, shot list, and style pass in the expandable sections, then copy or download the final script into your video tool

## Rate Limiting

The shared API key is protected by per-session limits: **5 generations per browser session** with a **60-second cooldown** between runs. The remaining quota is shown in the sidebar; refreshing the page starts a new session. Adjust `MAX_GENERATIONS_PER_SESSION` and `COOLDOWN_SECONDS` at the top of `robinframe.py`.
